...
const plugin = require('indexer-plugin/src/controller/indexController');
...
module.exports = (opts = {}) => {
    ...
    router
        ...
        .post('/index', async (ctx, next) => {
            await plugin.saveIndex(ctx, next)
        })
        .get('/index', async (ctx, next) => {
             await plugin.fetchIndex(ctx, next)
        })
        .delete('/index', async (ctx, next) => {
             await plugin.deleteIndex(ctx, next)
        })
        .put('/index', async (ctx, next) => {
             await plugin.updateIndex(ctx, next)
        })
    return router;
};
